    ...
    for node_id in graph:
        ...
        if node['defaulted'] != 1:
            ...
            for neighbor in neighbor_nodes:
                if neighbor['defaulted'] == 1 and neighbor['id'] in origins:
...
def check_infected_neighbours_recursive(graph: DiGraph, node_id, already_considerer_nodes=None):
    if already_considerer_nodes is None:
        already_considerer_nodes = []
    s = 0
    already_considerer_nodes.append(node_id)
    neighbors = graph[node_id]
    neighbor_nodes = [graph.nodes[node_id] for node_id in list(neighbors)]
    for neighbor in neighbor_nodes:
        if neighbor['defaulted'] == 1 and neighbor['id'] not in already_considerer_nodes:
...
def check_infected_neighbours(graph: DiGraph, node_id):
    ...
    neighbor_nodes = [graph.nodes[node_id] for node_id in list(neighbors)]
    for neighbor in neighbor_nodes:
        if neighbor['defaulted'] == 1 and neighbor['id'] != node_id:
